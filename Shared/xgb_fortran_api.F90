...
! 2022-02-21 - michael.manyin@nasa.gov - Added XGDMatrixFree_c and _f
! ----------------------------------------------------------------------------
 use iso_c_binding
 implicit none
...
!!!  MEM
interface
  integer(kind=c_int) function XGDMatrixFree_c(handle) bind(C, name="XGDMatrixFree")
...
!!!  MEM
  integer(kind=c_int) function XGDMatrixFree_f(handle) result(rc)
   use iso_c_binding, only: c_int, c_ptr
   type(c_ptr), value           :: handle ! DMatrixHandle
   ! starts here
   rc = XGDMatrixFree_c(handle)
  end function
